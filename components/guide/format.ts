 ...
 * `lib/core/render.ts`, and a test asserting the two agreed pinned the symptom while leaving two
 * copies to edit. The definitions now live in `lib/core/guide.ts`; this file stays the page's
 * formatting surface, so nothing that imports them from here had to move.
 */
import { count, ioText, mixerText, num } from '@/lib/core'

export { count, ioText, mixerText, num }
