 ...
 */
describe('Score (§7.1)', () => {
  it('is a variadic miss prefix followed by exactly five fixed keys', () => {
    // No priority levels: the five tail keys alone are a valid vector.
    expectTypeOf<[number, number, number, number, number]>().toExtend<Score>()
    // Two priority levels.
  ...
  it('is integers throughout, so comparison is exact (invariant 6)', () => {
    ...
    expectTypeOf<[string, number, number, number, number]>().not.toExtend<Score>()
  })
})
