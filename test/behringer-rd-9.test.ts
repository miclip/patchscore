 ...
 *
 * p.10: *"DECAY controls how long the drum will ring. Turn CW for longer tones"*. The bass drum
...
describe('sustain claims (§3/#506)', () => {
  ...
  it('declares `decays` on exactly these recipes, off the page that says so', () => {
    ...
    expect(declared.map((r) => r.id)).toEqual(['rd9-sub-dark'])
    for (const r of declared) {
      expect(r.sustain, r.id).toEqual({
        kind: 'decays',
        control: { kind: 'parameters', params: ['DECAY'] },
        evidence: { kind: 'manual', source: 'RD-9 User Manual V 1.0, p.10' },
      })
