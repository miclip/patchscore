...
describe('no other device is affected (#385)', () => {
  it('lists every device in the library authoring a module', () => {
    ...
    // third, the minilogue xd the fourth and the Matriarch the fifth, and this list is the
    // standing count of how far it has gone. Every device not named here renders exactly as it
    // did, which is what the goldens then prove byte for byte.
...
describe('hoistOrder keeps the legacy sort where nothing is moduled (§7.2)', () => {
  ...
  it('sorts by name in code unit order when no parameter carries a module', () => {
    // The order every guide in the library renders today. Forty-one devices author no module
