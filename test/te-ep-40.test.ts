...
describe('trigger notes: read for, and declined (§2.1/#334)', () => {
   ...
   */
  it('leaves 390 grid parts blank, and pins how many there are', () => {
    const { grid } = sweep()

    // 228 until #345 borrowed the pool's last five unserved roles; 252 until Hard Techno, ten
    // parts this box carries whole; 330 until Drum and Bass, whose kick, snare, closed hat, open
    // hat and ghost perc this box takes on every seed, the dark open hat answering a `dirty`
    // request at sqrt(2).
  ...
  it('accounts for every part that draws no grid, by which reason', () => {
    ...
    // classify — which is what would make the 390 above an undercount rather than a measurement.
    let assignments = 0
    for (const template of TEMPLATES) {
      for (const seed of SEEDS) {
