...
describe('T-1 manifest', () => {
  ...
  // -------------------------------------------------------------------------
  // §3.3 — the gap: a box that drives analog voices and is not offered as doing it
  //
  // Two independent things stop it, and these two tests keep them apart on purpose. Fixing
  // them as one is the mistake the manifest's head note warns about, so whichever is repaired
  // first should move exactly one of these and leave the other red for its own reason.
  // -------------------------------------------------------------------------
  ...
  describe('voice control (§3.3)', () => {
    it('offers no pitch-and-gate bundle, because Function decides what a CV out carries', () => {
      ...
      // default, so `signal` is all three and §3.3's sole-kind rule excludes them.
      //
      // **This is not the Cascadia's case wearing a new name, and recording it as one would
      // have buried the finding.** That box's `['gate','trigger']` outputs are *ambiguous* —
      // trigger by default, gate only if a global setting changes — and ranking on membership
      // told a reader to play a synth from an end-of-attack pulse. These are *configurable*: a
      // socket somebody sets, by a named control, in a documented place. `JackSignalKind` has
      // one word for both, and whether it should is a question about the shared vocabulary
    ...
    it('would still find no bundle if the CV outputs were single-kind, and that is an engine finding', () => {
      ...
      //
      // **This is a second blocker, not a consequence of the first**, and the two assertions
      // below are what proves it: give the sockets a single kind and the pairing *still* fails,
      // renumber them as well and it succeeds. So the `signal` question and the ordinal reader
      // are independent, and repairing either alone leaves the box undriven.
      //
      // This one is a bug in the reader rather than a shape the model cannot hold, and the
      // engine is not touched here (skill §6, #57): `CLAUDE.md` puts the fix before the device
      // rather than after. The test is the evidence for the claim, not an assertion that it
      // should stay true — when #201's reader learns a letter, this is the test that changes.
