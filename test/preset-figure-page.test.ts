    ...
    for (const [name, markup] of surfaces) {
      for (const match of markup.matchAll(/href="([^"]+)"/g)) {
        ...
        expect(href, `${name} links to ${href}`).not.toMatch(
          /^\/riffs\/(?!acid-tracks-line$|an-ending-ascent-pad$|blade-runner-blues-lead$|blue-monday-bass$|i-feel-love-one-shape-arp$|inner-city-life-held-sub$|show-me-love-organ-stab$|strings-of-life-walking-entry-stab$|tears-in-rain-held-second$|thriller-synth-riff$)/,
        )
      }
    }
