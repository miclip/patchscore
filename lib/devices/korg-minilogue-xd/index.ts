...
const recipes: Recipe[] = [
  ...
  /**
   * The dark stab, and it is the *filter* that makes it dark rather than the name on it. p.23
   * opens the FILTER section with it: *"The low-pass filter creates a brighter or darker sound
   * by selectively filtering certain parts of the harmonic spectrum of the sound generated by
   * the oscillators and the noise generator."* So this recipe takes the same four voices the
   * clean stab uses and moves three things:
   *
   *  - **the oscillators sit lower.** Two saws instead of a square and a triangle, and VCO 2 is
   *    an octave down at 16' and louder than VCO 1, so the chord's weight is under the voicing
   *    rather than in it. Saw is what gives the low-pass something to take away.
   *  - **the noise closes with them.** `noiseEngine('Low', 900)` against the clean stab's 6000,
   *    on the one scale p.20 prints for `Low` — the same knob, the same 10 Hz–21 kHz CUTOFF,
   *    set a fraction of the way up it. The multi engine is a thump under the chord, not air
   *    over it.
   *  - **CUTOFF 330 against 720, and RESONANCE stays at 160.** Darkness is the corner frequency
   *    doing the work; resonance is this device's grit axis, and lifting it here would take the
   *    patch towards `dirty` (620) instead. DRIVE is at 0% for the same reason.
   *
   * `KEYTRACK 100%` is the one that keeps a four-note chord playable at a cutoff this low: p.23
   * has it moving the cutoff with the key, centred on C4, so the top of the voicing does not
   * disappear behind a filter set for the bottom of it.
   */
  {
    id: 'mxd-stab-dark',
    role: 'stab',
    ...
    voice: 'voice',
    verified: false,
    title: 'Four-voice chord heavily low-passed, the weight an octave underneath it',
    params: [
      ...program(0, 0),
      ...poly(),
      ...vco1('SAW', "8'", 0, 320),
      ...vco2('SAW', "16'", -7, 260, 'OFF', 'OFF', 0),
      ...noiseEngine('Low', 900),
      ...mix(620, 740, 90),
      ...filt(330, 160, '0%', '100%'),
      ...ampEg(0, 360, 90, 320),
      ...eg(0, 280, 18, 'CUTOFF'),
      ...lfo('TRI', 'NORMAL', 130, 0, 'CUTOFF'),
      ...fx('REV', 'ON', 24),
    ],
  },
  {
