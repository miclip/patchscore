 ...
 *
 * ## One thing the documentation says that this file does not simply repeat
 *
 ...
 *
 * ## What this box does that the manifest cannot yet show
 *
 * **The T-1 drives analog voices, and nothing here says so.** That is a *gap*, not a reading,
 * and it is left standing as one. The pairing is real and the documentation prints it:
 * `/t1/appendix/t1-config/cv-gate/`'s 2-voices preset is *"assign CV A and Gate A to Channel 1"*,
 * and its 3-voices preset does the same across three. A reader with a Minitaur in front of them
 * can do this today. The guide will not offer it, because §3.3's voice-control pass cannot read
 * either half of what makes it true — and invariant 5's discipline is that a hole gets shown
 * rather than filled, so both declarations below stand as the document supports them.
 *
 * **Two things are in the way. They are not one problem and must not be fixed as one.**
 *
 * **First, `cv · a`-`cv · d` declare three kinds, because a setting chooses.** T1 Config gives
 * each CV output a `Function` of `Pitch`, `Velocity` or `Gate`, and no page states a default, so
 * `['pitch-cv', 'cv', 'gate']` is what the socket carries across its settings. §3.3's sole-kind
 * rule then excludes them from primary voice-control bundles.
 *
 * That rule is doing something here it was not written for, and the difference is worth stating
 * rather than filing under precedent. The Cascadia's `['gate', 'trigger']` end-of-stage outputs
 * are **ambiguous**: its page says trigger by default and gate only if a global setting changes,
 * so ranking on membership alone told a reader to play a synth from an end-of-attack pulse. These
 * are **configurable** — a socket set to a kind, by a named control, in a documented place, which
 * is exactly what this file's note on `cv · a` tells a reader to do. A socket somebody sets and a
 * socket nobody has pinned down are different claims, and `JackSignalKind` has one word for both.
 * Whether it should is a question about the shared vocabulary rather than about this device, and
 * it is deliberately not answered in a device folder.
 *
 * **Second, and separately, the pairing is lettered.** #201 pairs a multitrack CV sequencer's
 * pitch and gate sockets by *trailing ordinal* — the Hapax's `Cv out 1` with `gate out 1`. This
 * box numbers nothing: the silkscreen reads `cv a`-`cv d` and `gate a`-`gate b`. Same rule, same
 * ordinal shared across two sibling groups, with a letter where that one assumed a digit. It is a
 * bug in the reader rather than a shape the model cannot hold.
 *
 * **Neither is worked around, and the engine is not touched** (skill §6, #57). `CLAUDE.md` gives
 * the reason as a rule: if adding a device requires touching the engine, the abstraction is wrong
 * and the fix belongs before the device rather than after — and a device commit is the worst
 * place to settle the first of these two, because the pressure to finish a device is how a
 * shortcut gets taken and then inherited. So the three kinds stay, being true, and the letters
 * stay, being what the panel says. Softening either to slip past the rule would buy a rendered
 * cable with an invented fact underneath it.
 *
 * `test/torso-t1.test.ts` pins the two halves separately rather than asserting the outcome, so
 * whichever is repaired first has a test that moves for the right reason.
 *
