 ...
 *
 * **At `/studio` rather than at `/`.** It was the front door while it was the whole product, and
 * what it is now is one of several things a reader might want and the only one that asks for a
 * rig before it does anything. `/` is an orientation page; this is the app. Nothing about the
 * route changed with the address: the decode, the first-frame byte equality with the client
 * (#99) and #44's one-canonical rule are all as they were, and `lib/studio/session.ts` writes
 * the address bar from `location.pathname`, so the sync never named `/` in the first place.
 *
 * Legacy permalinks to `/?format=…` are three query-sensitive rules in `next.config.ts`.
 */
 ...
 *
 * `canonical: '/studio'` is stated here rather than inherited (#44). The layout's default is `/`
 * and that is now a real page, so this route has to name its own or every guide would claim to
 * be the orientation page. It is also the one route where per-view metadata and one canonical
 * URL sit side by side, and the pairing is the decision worth being able to read in one place —
 * and to test in one place.
 *
